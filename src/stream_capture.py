...
class StreamCapture:
    ...
    def capture_packets(self):
        def packet_callback(pkt):
            ...
            try:
                if TCP in pkt and Raw in pkt and pkt[TCP].dport == 1935:  # RTMP默认端口
                    # 尝试多种编码方式解码
                    raw_data = None
                    for encoding in ['utf-8', 'gbk', 'ascii']:
                        try:
                            raw_data = pkt[Raw].load.decode(encoding, errors='ignore')
                            break
                        except:
                            continue
                    ...
                    # 检查是否包含connect命令（两种情况）
                    if ('connect' in raw_data and 'tcUrl' in raw_data) or \
                       ('Set Chunk Size 4096' in raw_data and 'connect()' in raw_data):
                        # 匹配RTMP服务器地址
                        server_match = re.search(r'rtmp://[^\s\'"]+douyincdn\.com/third', raw_data)
                        if server_match:
                            server_url = server_match.group(0)
                            self.current_server = server_url
                            self.log_result(f"找到RTMP服务器地址: {server_url}")

                    # 检查是否包含releaseStream命令
                    elif 'releaseStream' in raw_data:
                        # 匹配串流密钥（更新后的模式）
                        stream_key_patterns = [
                            r'stream-\d+\?auth_key=[0-9a-f-]+',  # 原有格式
                            r'stream-\d+\?expire=\d+&sign=[0-9a-f]+',  # 新格式
                            r'stream-\d+\?[^\s\'"]+',  # 通用格式（如果上面都没匹配到）
                        ]
                        
                        stream_key = None
                        for pattern in stream_key_patterns:
                            stream_key_match = re.search(pattern, raw_data)
                            if stream_key_match:
                                stream_key = stream_key_match.group(0)
                                break
                            
                        if stream_key and hasattr(self, 'current_server'):
                            # 构建完整推流地址
                            full_url = f"{self.current_server}/{stream_key}"
                            
                            # 检查是否已经捕获过
                            if full_url not in self.captured_streams:
                                self.captured_streams.add(full_url)
                                self.log_result(
                                    f"找到完整推流信息：\n"
                                    f"服务器：{self.current_server}\n"
                                    f"串流密钥：{stream_key}\n"
                                    f"完整地址：{full_url}\n"
                                )
                ...
                self.log_result(f"解析数据包时出错: {str(e)}\n")
                        
        try:
            # 获取选中的网卡
            iface = self.get_selected_interface()
            self.log_result(f"开始在网卡 {iface} 上捕获...\n")
            
            # 只捕获RTMP流量
            sniff(
                iface=iface,
                filter="tcp port 1935",  # 只过滤RTMP端口
                prn=packet_callback,
                store=0
            )
        except Exception as e:
            self.log_result(f"捕获错误: {str(e)}\n")
            self.stop_capture()
